...
# [**Basic Enemy AI**](SDLProject/main.cpp)

### 🇫🇷 26 Messidor, Year CCXXX 🇫🇷

***Song of the day***: _[**La Marseillaise s’empare de Paris**](https://youtu.be/WBNjyvbqRYY) performed by L'Orchestre National de France (2021)._
...
void update()
{
    while (delta_time >= FIXED_TIMESTEP) {
        for (int i = 0; i < ENEMY_COUNT; i++) state.enemies[i].update(FIXED_TIMESTEP, state.platforms, PLATFORM_COUNT);
    }
}
...
<sub>**Code Blocks 2, 3, and 4**: Turning our entities into finite state machines.</sub>
...
void initialise()
{
    ...
    state.enemies[0].set_ai_state(IDLE);  // start at rest
}
