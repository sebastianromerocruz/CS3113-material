...
![supermario](assets/papermario.gif)
...
We'll be using a simple texture for our cube, making it look like a [**crate**](assets/crate0_diffuse.png).
...
{
    this->model_matrix = glm::rotate(this->model_matrix, glm::radians(this->rotation.x), glm::vec3(1.0, 0.0f, 0.0f));
}
```
```c++
void initialise()
...
![crate45rot](assets/crate45rot.png)
...
{
    if (this->entity_type == CUBE) this->rotation.x += this->ROTATION_SPEED * delta_time;
}
```
```c++
void initialise()
