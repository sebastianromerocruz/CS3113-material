...
void Effects::start(EffectType effect_type, float effect_speed)
{
    ...
    switch (m_current_effect)
    {
        ...
        case SHAKE:   m_time_left = 1.0f;  break;       // Set duration to 1s
        // ———————————————————————————————————————————————————————————————— //
    }
}
...
void Effects::update(float delta_time)
{
   switch (m_current_effect)
   {
       ...
       case SHAKE:
           // 1. Update remaining time for your effect
           m_time_left -= delta_time * m_effect_speed;
           
           // 2. If out of time, reset m_view_offset and m_current_effect
           if (m_time_left <= 0.0f)
           {
               m_view_offset = glm::vec3(0.0f, 0.0f, 0.0f);
               m_current_effect = NONE;
           }
           // 3. Else, update m_view_offset
           else
           {
               float min = -0.1f;
               float max = 0.0f;
