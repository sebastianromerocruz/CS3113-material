...
* CS 3113: Animation Exercise (SOLUTION; copy and paste this code onto main.cpp to test)
* 3 Messidor, Year CCXXXI
...
void initialise()
{
    ...
    g_character_program.SetProjectionMatrix(g_projection_matrix);
    g_character_program.SetViewMatrix(g_view_matrix);
    
    glUseProgram(g_character_program.programID);
    g_character_texture_id = load_texture(PLAYER_SPRITE_FILEPATH);
    
    // ————— FRAME ———— //
    g_frame_model_matrix     = glm::mat4(1.0f);
    g_frame_program.SetProjectionMatrix(g_projection_matrix);
    g_frame_program.SetViewMatrix(g_view_matrix);
    
    glUseProgram(g_frame_program.programID);
...
void update()
{
    ...
    if (g_is_spinning)
    {
        ...
        if (g_animation_time >= seconds_per_frame)
        {
            ...
            if (g_animation_index >= CHARACTER_SHEET_COLS)
            {
                g_animation_index = 0;
            }
        }
...
void render() {
    ...
    g_character_program.SetModelMatrix(g_character_model_matrix);
    
    // ———— PART 4 ———— //
    draw_sprite_from_texture_atlas(&g_character_program, g_character_texture_id, g_animation_index, CHARACTER_SHEET_ROWS, CHARACTER_SHEET_COLS);
    // ———— PART 4 ———— //
    
    g_frame_program.SetModelMatrix(g_frame_model_matrix);
