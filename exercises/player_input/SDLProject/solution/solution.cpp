...
// Boolean for extra credit
bool g_is_rotating   = true;
bool g_rot_clockwise = true;
// ———————————————— PART 1 ———————————————— //
        ...
        switch (event.type)
        {
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym)
                {
                    ...
                    case SDLK_r:
                        g_is_rotating = true;
                        g_rot_clockwise = true;
                        break;
                        
                    // Rotating counter-clockwise
                    case SDLK_c:
                        g_is_rotating = true;
                        g_rot_clockwise = false;
                        break;
...
void update()
{
    ...
    // ———————————————— PART 3 ———————————————— //
    // Update translation
    if (g_position.x >= 3.0f or g_position.x <= -3.0f) g_movement.x *= -1.0f;
    g_position += g_movement * TRANS_SPEED * delta_time;
    // Update rotation
    if (g_is_rotating)
    {
        if (g_rot_clockwise) g_rot_angle += ROT_SPEED * delta_time;
        else g_rot_angle -= ROT_SPEED * delta_time;
    }
